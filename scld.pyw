...
try:
    ...
    from yt_dlp.utils import OUTTMPL_TYPES, PostProcessingError, date_from_str, locked_file, replace_extension, variadic
...
class GUIYTLogger:
    ...
    def __init__(self, log_cb, debug_mode=False, hide_warnings=False):
        self.log_cb = log_cb
...
def search_soundcloud_url(client: SoundCloud, query: str, log_fn=None) -> Optional[str]:
    try:
    ...
    def _download_worker(self, scdl_args: dict):
        try:
            ...
            if not client.is_client_id_valid():
                self.root.after(0, self._log, "🔑 Client ID chưa có hoặc đã hết hạn, đang tạo Client ID động...")
                client = SoundCloud(None, token if token else None)
                if not client.is_client_id_valid():
                    raise RuntimeError("Không thể tạo Client ID động từ SoundCloud.")
                self.root.after(0, self._log, f"✅ Đã tạo Client ID động thành công: {client.client_id}")
            ...
            if scdl_args["mode"] == "me":
                ...
                resolved_url = search_soundcloud_url(client, url, log_fn=lambda m: self.root.after(0, self._log, m))
                if not resolved_url:
            ...
            def progress_hook(d):
                ...
                if status == "downloading":
                    ...
                    self.root.after(0, self._update_progress, filename, p_val, percent_str, speed_str, eta_str)
                elif status == "finished":
                    filename = os.path.basename(d.get("filename", ""))
                    self.root.after(0, self._update_track_finished, filename)
            ...
            self.root.after(0, self._on_download_complete, True, "✅ Quá trình tải hoàn tất thành công!")
        except AbortDownloadException:
            self.root.after(0, self._on_download_complete, False, "⏹ Tiến trình đã được dừng theo yêu cầu của bạn.")
        except Exception as e:
            err_msg = str(e)
            if "AbortDownloadException" in err_msg or self.abort_event.is_set():
                self.root.after(0, self._on_download_complete, False, "⏹ Tiến trình đã được dừng theo yêu cầu của bạn.")
            else:
                self.root.after(0, self._on_download_complete, False, f"❌ Lỗi: {err_msg}")
